...
def _resolve_room_or_404(db: DbSession, room_slug: str):
...
def get_gallery_images(db: DbSession) -> list[GalleryImageOut]:
    default_room = get_or_create_default_room(db)
...
def get_gallery_images_for_room(room_slug: PublicIdPath, db: DbSession) -> list[GalleryImageOut]:
    room = _resolve_room_or_404(db, room_slug)
...
def get_job_status_by_hash(jpg_hash: PublicIdPath, db: DbSession) -> JobStatusOut:
    default_room = get_or_create_default_room(db)
...
def get_job_status_for_room(room_slug: PublicIdPath, job_id: int, db: DbSession) -> JobStatusOut:
    room = _resolve_room_or_404(db, room_slug)
    job = get_job_or_404(db, job_id)
    if job is None or job.room_id != room.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="job not found")
...
def download_qr(job_id: int, request: Request, db: DbSession) -> Response:
    default_room = get_or_create_default_room(db)
...
def download_qr_for_room(job_id: int, room_slug: PublicIdPath, request: Request, db: DbSession) -> Response:
    room = _resolve_room_or_404(db, room_slug)
    job = get_completed_job_or_404(db, job_id)
    if job is None or job.room_id != room.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="job not completed")
...
def download_result_by_qr_hash(qr_hash: str, db: DbSession) -> FileResponse:
