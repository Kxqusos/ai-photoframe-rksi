...
from photoframe_backend.api.http.schemas.media import MediaUploadOut
...
@router.post("/prompt-preview", status_code=status.HTTP_201_CREATED, response_model=MediaUploadOut)
async def upload_prompt_preview(file: UploadFile) -> MediaUploadOut:
    return MediaUploadOut(url=await save_prompt_preview(file))


@router.post("/prompt-icon", status_code=status.HTTP_201_CREATED, response_model=MediaUploadOut)
async def upload_prompt_icon(file: UploadFile) -> MediaUploadOut:
    return MediaUploadOut(url=await save_prompt_icon(file))
